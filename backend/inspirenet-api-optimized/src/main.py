...
async def startup_probe():
    ...
    Cloud Run will not route traffic to this instance until this endpoint returns 200.
    This prevents user requests from hitting the instance during model loading.

    Returns:
        503: If model is not yet loaded (Cloud Run will keep checking)
        200: When model is ready and instance can accept traffic
    """
    ...
    # Model is ready, instance can accept traffic
    return {
        "status": "ready",
        "model_loaded": True,
        "device": str(processor.device) if processor else None,
